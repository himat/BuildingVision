...
filetype = ".jpg"
edges_files_path = os.path.join(dir, input_dir)
...
(edges_image_name,) = tf.train.slice_input_producer([edges_filenames_tf])
...
edges_images_batch = tf.train.batch([edges_image],
    batch_size=mb_size,
    capacity=30,
    num_threads=mb_size)
...
with tf.Session() as sess:
    ...
    X_edges_batch = sess.run(edges_images_batch)
    ...
    for batch_idx in range(num_data // mb_size):
        ...
        for i, img in enumerate(produced_image_batch):
            plot_save_single(img, save_only=True, dir=test_save_dir, name="img"+str(batch_idx)+str(i))
